...
"@babel/preset-env":"^7.12.1","@ffmpeg/core":"^0.10.0","@types/emscripten":"^1.39.4","babel-loader":"^8.1.0",chai:"^4.2.0",cors:"^2.8.5",eslint:"^7.12.1","eslint-config-airbnb-base":"^14.1.0","eslint-plugin-import":"^2.22.1",express:"^4.17.1",mocha:"^8.2.1","mocha-headless-chrome":"^2.0.3","npm-run-all":"^4.1.5","wait-on":"^5.3.0",webpack:"^5.3.2","webpack-cli":"^4.1.0","webpack-dev-middleware":"^4.0.0"}}},{}],3:[function(x,J,D){D=x("_process");const C=x("resolve-url");({devDependencies:x}=x("../../package.json"));
J.exports={corePath:"development"===D.env.NODE_ENV?C("/node_modules/@ffmpeg/core/dist/ffmpeg-core.js"):`https://unpkg.com/@ffmpeg/core@${x["@ffmpeg/core"].substring(1)}/dist/ffmpeg-core.js`}},{"../../package.json":2,_process:27,"resolve-url":33}],4:[function(x,J,D){const C=x("resolve-url"),q=h=>new Promise((A,e)=>{const u=new FileReader;u.onload=()=>{A(u.result)};u.onerror=({target:{error:{code:v}}})=>{e(Error(`File could not be read! Code=${v}`))};u.readAsArrayBuffer(h)});J.exports=async h=>{let A=
h;if("undefined"===typeof h)return new Uint8Array;if("string"===typeof h)A=/data:_data\/([a-zA-Z]*);base64,([^"]*)/.test(h)?atob(h.split(",")[1]).split("").map(e=>e.charCodeAt(0)):await (await fetch(C(h))).arrayBuffer();else if(h instanceof File||h instanceof Blob)A=await q(h);return new Uint8Array(A)}},{"resolve-url":33}],5:[function(x,J,D){const C=x("resolve-url"),{log:q}=x("../utils/log"),h=async(A,e)=>{q("info",`fetch ${A}`);const u=await (await fetch(A)).arrayBuffer();q("info",`${A} file size = ${u.byteLength} bytes`);
e=new Blob([u],{type:e});e=URL.createObjectURL(e);q("info",`${A} blob URL = ${e}`);return e};J.exports=async({corePath:A})=>{if("string"!==typeof A)throw Error("corePath should be a string!");A=C(A);const e=await h(A,"application/javascript"),u=await h(A.replace("ffmpeg-core.js","ffmpeg-core.wasm"),"application/wasm"),v=await h(A.replace("ffmpeg-core.js","ffmpeg-core.worker.js"),"application/javascript");if("undefined"===typeof createFFmpegCore)return new Promise(m=>{const g=document.createElement("script"),
f=()=>{g.removeEventListener("load",f);q("info","ffmpeg-core.js script loaded");m({createFFmpegCore,corePath:e,wasmPath:u,workerPath:v})};g.src=e;g.type="text/javascript";g.addEventListener("load",f);document.getElementsByTagName("head")[0].appendChild(g)});q("info","ffmpeg-core.js script is loaded already");return Promise.resolve({createFFmpegCore,corePath:e,wasmPath:u,workerPath:v})}},{"../utils/log":10,"resolve-url":33}],6:[function(x,J,D){D=x("./defaultOptions");const C=x("./getCreateFFmpegCore");
x=x("./fetchFile");J.exports={defaultOptions:D,getCreateFFmpegCore:C,fetchFile:x}},{"./defaultOptions":3,"./fetchFile":4,"./getCreateFFmpegCore":5}],7:[function(x,J,D){J.exports={defaultArgs:["./ffmpeg","-nostdin","-y"],baseOptions:{log:!1,logger:()=>{},progress:()=>{},corePath:""}}},{}],8:[function(x,J,D){const {defaultArgs:C,baseOptions:q}=x("./config"),{setLogging:h,setCustomLogger:A,log:e}=x("./utils/log"),u=x("./utils/parseProgress"),v=x("./utils/parseArgs"),{defaultOptions:m,getCreateFFmpegCore:g}=
x("./node"),{version:f}=x("../package.json"),l=Error("ffmpeg.wasm is not ready, make sure you have completed load().");J.exports=(E={})=>{const {log:t,logger:z,progress:n,...B}={...q,...m,...E};let p=null,w=null,G=null,H=!1,L=n;const I=({type:N,message:M})=>{e(N,M);u(M,L);"FFMPEG_END"===M&&null!==G&&(G(),G=null,H=!1)};h(t);A(z);e("info",`use ffmpeg.wasm v${f}`);return{setProgress:N=>{L=N},setLogger:N=>{A(N)},setLogging:h,load:async()=>{e("info","load ffmpeg-core");if(null===p){e("info","loading ffmpeg-core");
...
},isLoaded:()=>null!==p,run:(...N)=>{e("info",`run ffmpeg command: ${N.join(" ")}`);if(null===p)throw l;if(H)throw Error("ffmpeg.wasm can only run one command at a time");H=!0;return new Promise(M=>{const T=[...C,...N].filter(S=>0!==S.length);G=M;w(...v(p,T))})},exit:()=>{if(null===p)throw l;H=!1;p.exit(1);G=w=p=null},FS:(N,...M)=>{e("info",`run FS.${N} ${M.map(S=>"string"===typeof S?S:`<${S.length} bytes binary file>`).join(" ")}`);if(null===p)throw l;let T=null;try{T=p.FS[N](...M)}catch(S){if("readdir"===
N)throw Error(`ffmpeg.FS('readdir', '${M[0]}') error. Check if the path exists, ex: ffmpeg.FS('readdir', '/')`);if("readFile"===N)throw Error(`ffmpeg.FS('readFile', '${M[0]}') error. Check if the path exists`);throw Error("Oops, something went wrong in FS operation.");}return T}}}},{"../package.json":2,"./config":7,"./node":6,"./utils/log":10,"./utils/parseArgs":11,"./utils/parseProgress":12}],9:[function(x,J,D){x("regenerator-runtime/runtime");D=x("./createFFmpeg");({fetchFile:x}=x("./node"));J.exports=
{createFFmpeg:D,fetchFile:x}},{"./createFFmpeg":8,"./node":6,"regenerator-runtime/runtime":32}],10:[function(x,J,D){let C=!1,q=()=>{};J.exports={logging:C,setLogging:h=>{C=h},setCustomLogger:h=>{q=h},log:(h,A)=>{q({type:h,message:A});C&&console.log(`[${h}] ${A}`)}}},{}],11:[function(x,J,D){J.exports=(C,q)=>{const h=C._malloc(q.length*Uint32Array.BYTES_PER_ELEMENT);q.forEach((A,e)=>{const u=C._malloc(A.length+1);C.writeAsciiToMemory(A,u);C.setValue(h+Uint32Array.BYTES_PER_ELEMENT*e,u,"i32")});return[q.length,
h]}},{}],12:[function(x,J,D){let C=0,q=0;const h=A=>{const [e,u,v]=A.split(":");return 3600*parseFloat(e)+60*parseFloat(u)+parseFloat(v)};J.exports=(A,e)=>{if("string"===typeof A)if(A.startsWith("  Duration")){if(A=A.split(", ")[0].split(": ")[1],A=h(A),e({duration:A,ratio:q}),0===C||C>A)C=A}else A.startsWith("frame")||A.startsWith("size")?(A=A.split("time=")[1].split(" ")[0],A=h(A),q=A/C,e({ratio:q,time:A})):A.startsWith("video:")&&(e({ratio:1}),C=0)}},{}],13:[function(x,J,D){function C(e){var u=
e.length;if(0<u%4)throw Error("Invalid string. Length must be a multiple of 4");e=e.indexOf("=");-1===e&&(e=u);return[e,e===u?0:4-e%4]}D.byteLength=function(e){e=C(e);var u=e[1];return 3*(e[0]+u)/4-u};D.toByteArray=function(e){var u=C(e);var v=u[0];u=u[1];var m=new A(3*(v+u)/4-u),g=0,f=0<u?v-4:v,l;for(l=0;l<f;l+=4)v=h[e.charCodeAt(l)]<<18|h[e.charCodeAt(l+1)]<<12|h[e.charCodeAt(l+2)]<<6|h[e.charCodeAt(l+3)],m[g++]=v>>16&255,m[g++]=v>>8&255,m[g++]=v&255;2===u&&(v=h[e.charCodeAt(l)]<<2|h[e.charCodeAt(l+
1)]>>4,m[g++]=v&255);1===u&&(v=h[e.charCodeAt(l)]<<10|h[e.charCodeAt(l+1)]<<4|h[e.charCodeAt(l+2)]>>2,m[g++]=v>>8&255,m[g++]=v&255);return m};D.fromByteArray=function(e){for(var u=e.length,v=u%3,m=[],g=0,f=u-v;g<f;g+=16383){for(var l=m,E=l.push,t,z=e,n=g+16383>f?f:g+16383,B=[],p=g;p<n;p+=3)t=(z[p]<<16&16711680)+(z[p+1]<<8&65280)+(z[p+2]&255),B.push(q[t>>18&63]+q[t>>12&63]+q[t>>6&63]+q[t&63]);t=B.join("");E.call(l,t)}1===v?(e=e[u-1],m.push(q[e>>2]+q[e<<4&63]+"==")):2===v&&(e=(e[u-2]<<8)+e[u-1],m.push(q[e>>
10]+q[e>>4&63]+q[e<<2&63]+"="));return m.join("")};var q=[],h=[],A="undefined"!==typeof Uint8Array?Uint8Array:Array;for(x=0;64>x;++x)q[x]="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"[x],h["ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charCodeAt(x)]=x;h[45]=62;h[95]=63},{}],14:[function(x,J,D){},{}],15:[function(x,J,D){(function(C){(function(){function q(b){if(b>V)throw new RangeError('The value "'+b+'" is invalid for option "size"');b=new Uint8Array(b);b.__proto__=
h.prototype;return b}function h(b,a,d){if("number"===typeof b){if("string"===typeof a)throw new TypeError('The "string" argument must be of type string. Received type number');return u(b)}return A(b,a,d)}function A(b,a,d){if("string"===typeof b){var c=a;if("string"!==typeof c||""===c)c="utf8";if(!h.isEncoding(c))throw new TypeError("Unknown encoding: "+c);a=f(b,c)|0;d=q(a);b=d.write(b,c);b!==a&&(d=d.slice(0,b));return d}if(ArrayBuffer.isView(b))return v(b);if(null==b)throw TypeError("The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object. Received type "+
...
}function u(b){e(b);return q(0>b?0:g(b)|0)}function v(b){for(var a=0>b.length?0:g(b.length)|0,d=q(a),c=0;c<a;c+=1)d[c]=b[c]&255;return d}function m(b){if(h.isBuffer(b)){var a=g(b.length)|0,d=q(a);if(0===d.length)return d;b.copy(d,0,0,a);return d}if(void 0!==b.length)return(a="number"!==typeof b.length)||(a=b.length,a=a!==a),a?q(0):v(b);if("Buffer"===b.type&&Array.isArray(b.data))return v(b.data)}function g(b){if(b>=V)throw new RangeError("Attempt to allocate Buffer larger than maximum size: 0x"+V.toString(16)+
" bytes");return b|0}function f(b,a){if(h.isBuffer(b))return b.length;if(ArrayBuffer.isView(b)||T(b,ArrayBuffer))return b.byteLength;if("string"!==typeof b)throw new TypeError('The "string" argument must be one of type string, Buffer, or ArrayBuffer. Received type '+typeof b);var d=b.length,c=2<arguments.length&&!0===arguments[2];if(!c&&0===d)return 0;for(var k=!1;;)switch(a){case "ascii":case "latin1":case "binary":return d;case "utf8":case "utf-8":return L(b).length;case "ucs2":case "ucs-2":case "utf16le":case "utf-16le":return 2*
d;case "hex":return d>>>1;case "base64":return N(b).length;default:if(k)return c?-1:L(b).length;a=(""+a).toLowerCase();k=!0}}function l(b,a,d){var c=!1;if(void 0===a||0>a)a=0;if(a>this.length)return"";if(void 0===d||d>this.length)d=this.length;if(0>=d)return"";d>>>=0;a>>>=0;if(d<=a)return"";for(b||="utf8";;)switch(b){case "hex":b=a;a=d;d=this.length;if(!b||0>b)b=0;if(!a||0>a||a>d)a=d;c="";for(d=b;d<a;++d)b=c,c=this[d],c=16>c?"0"+c.toString(16):c.toString(16),c=b+c;return c;case "utf8":case "utf-8":return n(this,
...
E(this,a+2,a+5),E(this,a+3,a+4);return this};h.prototype.toString=function(){var b=this.length;return 0===b?"":0===arguments.length?n(this,0,b):l.apply(this,arguments)};h.prototype.toLocaleString=h.prototype.toString;h.prototype.equals=function(b){if(!h.isBuffer(b))throw new TypeError("Argument must be a Buffer");return this===b?!0:0===h.compare(this,b)};h.prototype.inspect=function(){var b=D.INSPECT_MAX_BYTES;var a=this.toString("hex",0,b).replace(/(.{2})/g,"$1 ").trim();this.length>b&&(a+=" ... ");
...
B}function u(n,B,p){n=n._events;if(void 0===n)return[];B=n[B];if(void 0===B)return[];if("function"===typeof B)return p?[B.listener||B]:[B];if(p)for(p=Array(B.length),n=0;n<p.length;++n)p[n]=B[n].listener||B[n];else p=m(B,B.length);return p}function v(n){var B=this._events;if(void 0!==B){n=B[n];if("function"===typeof n)return 1;if(void 0!==n)return n.length}return 0}function m(n,B){for(var p=Array(B),w=0;w<B;++w)p[w]=n[w];return p}function g(n,B,p){"function"===typeof n.on&&f(n,"error",B,p)}function f(n,
B,p,w){if("function"===typeof n.on)if(w.once)n.once(B,p);else n.on(B,p);else if("function"===typeof n.addEventListener)n.addEventListener(B,function L(H){w.once&&n.removeEventListener(B,L);p(H)});else throw new TypeError('The "emitter" argument must be of type EventEmitter. Received type '+typeof n);}var l=(x="object"===typeof Reflect?Reflect:null)&&"function"===typeof x.apply?x.apply:function(n,B,p){return Function.prototype.apply.call(n,B,p)};var E=x&&"function"===typeof x.ownKeys?x.ownKeys:Object.getOwnPropertySymbols?
function(n){return Object.getOwnPropertyNames(n).concat(Object.getOwnPropertySymbols(n))}:function(n){return Object.getOwnPropertyNames(n)};var t=Number.isNaN||function(n){return n!==n};J.exports=C;J.exports.once=function(n,B){return new Promise(function(p,w){function G(L){n.removeListener(B,H);w(L)}function H(){"function"===typeof n.removeListener&&n.removeListener("error",G);p([].slice.call(arguments))}f(n,B,H,{once:!0});"error"!==B&&g(n,G,{once:!0})})};C.EventEmitter=C;C.prototype._events=void 0;
...
h=x("url");x=J.exports;for(var A in q)q.hasOwnProperty(A)&&(x[A]=q[A]);x.request=function(e,u){e=C(e);return q.request.call(this,e,u)};x.get=function(e,u){e=C(e);return q.get.call(this,e,u)}},{http:51,url:72}],19:[function(x,J,D){D.read=function(C,q,h,A,e){var u=8*e-A-1;var v=(1<<u)-1,m=v>>1,g=-7;e=h?e-1:0;var f=h?-1:1,l=C[q+e];e+=f;h=l&(1<<-g)-1;l>>=-g;for(g+=u;0<g;h=256*h+C[q+e],e+=f,g-=8);u=h&(1<<-g)-1;h>>=-g;for(g+=A;0<g;u=256*u+C[q+e],e+=f,g-=8);if(0===h)h=1-m;else{if(h===v)return u?NaN:Infinity*
(l?-1:1);u+=Math.pow(2,A);h-=m}return(l?-1:1)*u*Math.pow(2,h-A)};D.write=function(C,q,h,A,e,u){var v,m=8*u-e-1,g=(1<<m)-1,f=g>>1,l=23===e?Math.pow(2,-24)-Math.pow(2,-77):0;u=A?0:u-1;var E=A?1:-1,t=0>q||0===q&&0>1/q?1:0;q=Math.abs(q);isNaN(q)||Infinity===q?(q=isNaN(q)?1:0,A=g):(A=Math.floor(Math.log(q)/Math.LN2),1>q*(v=Math.pow(2,-A))&&(A--,v*=2),q=1<=A+f?q+l/v:q+l*Math.pow(2,1-f),2<=q*v&&(A++,v/=2),A+f>=g?(q=0,A=g):1<=A+f?(q=(q*v-1)*Math.pow(2,e),A+=f):(q=q*Math.pow(2,f-1)*Math.pow(2,e),A=0));for(;8<=
e;C[h+u]=q&255,u+=E,q/=256,e-=8);A=A<<e|q;for(m+=e;0<m;C[h+u]=A&255,u+=E,A/=256,m-=8);C[h+u-E]|=128*t}},{}],20:[function(x,J,D){J.exports="function"===typeof Object.create?function(C,q){q&&(C.super_=q,C.prototype=Object.create(q.prototype,{constructor:{value:C,enumerable:!1,writable:!0,configurable:!0}}))}:function(C,q){if(q){C.super_=q;var h=function(){};h.prototype=q.prototype;C.prototype=new h;C.prototype.constructor=C}}},{}],21:[function(x,J,D){J=this&&this.__importDefault||function(e){return e&&
e.__esModule?e:{"default":e}};Object.defineProperty(D,"__esModule",{value:!0});const C=x("stream"),q=J(x("sax")),h=x("./parse-time");class A extends C.Writable{constructor(e){super();this._parser=q.default.createStream(!1,{lowercase:!0});this._parser.on("error",this.destroy.bind(this));let u,v=0,m=0,g,f,l,E,t,z=[],n=!1,B=!1,p,w,G;const H=I=>{const N={RepresentationID:e,Number:m,Time:v};return I.replace(/\$(\w+)\$/g,(M,T)=>`${N[T]}`)};this._parser.on("opentag",I=>{switch(I.name){case "mpd":v=I.attributes.availabilitystarttime?
(new Date(I.attributes.availabilitystarttime)).getTime():0;p="dynamic"!==I.attributes.type;break;case "period":m=0;f=1E3;l=E=0;t=[];w=0;G=h.durationStr(I.attributes.start)||0;break;case "segmentlist":m=parseInt(I.attributes.startnumber)||m;f=parseInt(I.attributes.timescale)||f;E=parseInt(I.attributes.duration)||E;l=parseInt(I.attributes.presentationtimeoffset)||l;break;case "segmenttemplate":g=I.attributes;m=parseInt(I.attributes.startnumber)||m;f=parseInt(I.attributes.timescale)||f;break;case "segmenttimeline":case "baseurl":u=
I.name;break;case "s":z.push({duration:parseInt(I.attributes.d),repeat:parseInt(I.attributes.r),time:parseInt(I.attributes.t)});break;case "adaptationset":case "representation":w++;e||(e=I.attributes.id);if(n=I.attributes.id===`${e}`)G&&(v+=G),l&&(v-=l/f*1E3),this.emit("starttime",v);break;case "initialization":n&&this.emit("item",{url:t.filter(M=>!!M).join("")+I.attributes.sourceurl,seq:m,init:!0,duration:0});break;case "segmenturl":if(n){B=!0;var N=z.shift();N=((null===N||void 0===N?void 0:N.duration)||
E)/f*1E3;this.emit("item",{url:t.filter(M=>!!M).join("")+I.attributes.media,seq:m++,duration:N});v+=N}}});const L=()=>{p&&this.emit("endlist");n?this.emit("end"):this.destroy(Error(`Representation '${e}' not found`))};this._parser.on("closetag",I=>{switch(I){case "adaptationset":case "representation":w--;if(g&&z.length){B=!0;g.initialization&&this.emit("item",{url:t.filter(N=>!!N).join("")+H(g.initialization),seq:m,init:!0,duration:0});for(let {duration:N,repeat:M,time:T}of z)for(N=N/f*1E3,M=M||1,
v=T||v,I=0;I<M;I++)this.emit("item",{url:t.filter(S=>!!S).join("")+H(g.media),seq:m++,duration:N}),v+=N}B&&(this.emit("endearly"),L(),this._parser.removeAllListeners(),this.removeAllListeners("finish"))}});this._parser.on("text",I=>{"baseurl"===u&&(t[w]=I,u=null)});this.on("finish",L)}_write(e,u,v){this._parser.write(e);v()}}D.default=A},{"./parse-time":24,sax:35,stream:36}],22:[function(x,J,D){var C=this&&this.__importDefault||function(v){return v&&v.__esModule?v:{"default":v}};const q=x("stream"),
h=C(x("miniget"));D=C(x("./m3u8-parser"));C=C(x("./dash-mpd-parser"));const A=x("./queue"),e=x("./parse-time"),u={m3u8:D.default,"dash-mpd":C.default};x=(v,m={})=>{const g=new q.PassThrough({highWaterMark:m.highWaterMark}),f=m.chunkReadahead||3,l=m.liveBuffer||2E4,E=m.requestOptions,t=u[m.parser||(/\.mpd$/.test(v)?"dash-mpd":"m3u8")];if(!t)throw TypeError(`parser '${m.parser}' not supported`);let z=0;"undefined"!==typeof m.begin&&(z="string"===typeof m.begin?e.humanStr(m.begin):Math.max(m.begin-l,
...
+R;g.emit("progress",{num:++w,size:R,duration:c.duration,url:c.url},H.total,G);k(null)})},{concurrency:f}),L=c=>{g.emit("error",c);g.end()};let I,N,M,T=!0,S=!1,O=!1,V;const W=c=>{B=null;c?L(c):!T&&!S&&!O&&H.tasks.length+H.active<=I?(c=Math.max(0,N-(Date.now()-V)),T=!0,M=setTimeout(d,c)):!S&&!O||H.tasks.length||H.active||g.end()};let U,b,a=0;const d=()=>{V=Date.now();U=h.default(v,E);U.on("error",L);n(U);const c=U.pipe(new t(m.id));c.on("starttime",P=>{a||(a=P,"string"===typeof m.begin&&0<=z&&(z+=
a))});c.on("endlist",()=>{O=!0});c.on("endearly",U.unpipe.bind(U,c));let k=[];const r=P=>{if(!P.init){if(P.seq<=b)return;b=P.seq}z=P.time;H.push(P,W);k.push(P)};let K=[],R=0;c.on("item",P=>{P=Object.assign({time:a},P);if(z<=P.time)r(P);else for(K.push(P),R+=P.duration;1<K.length&&R-K[0].duration>l;){const F=K.shift();R-=F.duration}a+=P.duration});c.on("end",()=>{U=null;!k.length&&K.length&&K.forEach(P=>{r(P)});I=Math.max(1,Math.ceil(.01*k.length));N=k.reduce((P,F)=>F.duration+P,0);T=!1;W(null)})};
d();g.end=()=>{S=!0;p.die();H.die();clearTimeout(M);null===U||void 0===U?void 0:U.destroy();null===B||void 0===B?void 0:B.destroy();q.PassThrough.prototype.end.call(g,null);return g};return g};x.parseTimestamp=e.humanStr;J.exports=x},{"./dash-mpd-parser":21,"./m3u8-parser":23,"./parse-time":24,"./queue":25,miniget:26,stream:36}],23:[function(x,J,D){Object.defineProperty(D,"__esModule",{value:!0});x=x("stream");class C extends x.Writable{constructor(){super();this._lastLine="";this._seq=0;this._nextItemRange=
...
parseFloat(q.split(",")[0]));break;case "EXT-X-ENDLIST":this.emit("endlist")}else!/^#/.test(q)&&q.trim()&&(this.emit("item",{url:q.trim(),seq:this._seq++,duration:this._nextItemDuration,range:this._nextItemRange}),this._nextItemRange=null)}_write(q,h,A){let e=q.toString("utf8").split("\n");this._lastLine&&(e[0]=this._lastLine+e[0]);e.forEach((u,v)=>{this.destroyed||(v<e.length-1?this._parseLine(u):this._lastLine=u)});A()}}D.default=C},{stream:36}],24:[function(x,J,D){Object.defineProperty(D,"__esModule",
{value:!0});D.durationStr=D.humanStr=void 0;const C=/^\d+$/,q=/^(?:(?:(\d+):)?(\d{1,2}):)?(\d{1,2})(?:\.(\d{3}))?$/,h={ms:1,s:1E3,m:6E4,h:36E5};D.humanStr=A=>{if("number"===typeof A)return A;if(C.test(A))return+A;var e=q.exec(A);if(e)return+(e[1]||0)*h.h+ +(e[2]||0)*h.m+ +e[3]*h.s+ +(e[4]||0);e=0;const u=/(-?\d+)(ms|s|m|h)/g;let v;for(;null!==(v=u.exec(A));)e+=+v[1]*h[v[2]];return e};D.durationStr=A=>{let e=0;const u=/(\d+(?:\.\d+)?)(S|M|H)/g;let v;for(;null!==(v=u.exec(A));)e+=+v[1]*h[v[2].toLowerCase()];
return e}},{}],25:[function(x,J,D){Object.defineProperty(D,"__esModule",{value:!0});D.Queue=void 0;class C{constructor(q,h={}){this._worker=q;this._concurrency=h.concurrency||1;this.tasks=[];this.active=this.total=0}push(q,h){this.tasks.push({item:q,callback:h});this.total++;this._next()}_next(){if(!(this.active>=this._concurrency)&&this.tasks.length){var {item:q,callback:h}=this.tasks.shift(),A=!1;this.active++;this._worker(q,(e,u)=>{A||(this.active--,A=!0,null===h||void 0===h?void 0:h(e,u),this._next())})}}die(){this.tasks=
[]}}D.Queue=C},{}],26:[function(x,J,D){(function(C){(function(){function q(l,E={}){var t;const z=Object.assign({},q.defaultOptions,E),n=new e.PassThrough({highWaterMark:z.highWaterMark});n.destroyed=n.aborted=!1;let B,p,w,G=0,H=0,L,I=0,N,M=!1,T=0,S,O=0;if(null===(t=z.headers)||void 0===t?0:t.Range)if(t=/bytes=(\d+)-(\d+)?/.exec(`${z.headers.Range}`))T=parseInt(t[1],10),S=parseInt(t[2],10);z.acceptEncoding&&(z.headers=Object.assign({"Accept-Encoding":Object.keys(z.acceptEncoding).join(", ")},z.headers));
const V=c=>"HEAD"!==E.method&&M&&O!==N&&I++<z.maxReconnects?(w=null,H=0,L=setTimeout(b,Math.min(z.backoff.inc,z.backoff.max)),n.emit("reconnect",I,c),!0):!1,W=c=>n.destroyed?!1:w&&0<O?V(c.err):(!c.err||"ENOTFOUND"===c.err.message)&&H++<z.maxRetries?(L=setTimeout(b,c.retryAfter||Math.min(H*z.backoff.inc,z.backoff.max)),n.emit("retry",H,c.err),!0):!1,U=(c,k)=>{for(let r of k)c.on(r,n.emit.bind(n,r))},b=()=>{let c={},k;try{let y="string"===typeof l?new URL(l):l;c=Object.assign({},{host:y.host,hostname:y.hostname,
...
y=>{n.destroyed||n.readableEnded||(R(),W({err:y})?B.removeListener("close",K):n.emit("error",y))},K=()=>{R();W({})},R=()=>{B.removeListener("close",K);null===p||void 0===p?void 0:p.removeListener("data",P);null===w||void 0===w?void 0:w.removeListener("end",F)},P=y=>{O+=y.length},F=()=>{R();V()||n.end()};B=k.request(c,y=>{if(!n.destroyed)if(v.has(y.statusCode)){if(G++>=z.maxRedirects)n.emit("error",new q.MinigetError("Too many redirects"));else{if(y.headers.location)l=y.headers.location;else{y=new q.MinigetError("Redirect status code given with no location",
y.statusCode);n.emit("error",y);R();return}setTimeout(b,1E3*parseInt(y.headers["retry-after"]||"0",10));n.emit("redirect",l)}R()}else if(m.has(y.statusCode))W({retryAfter:parseInt(y.headers["retry-after"]||"0",10)})||(y=new q.MinigetError(`Status code: ${y.statusCode}`,y.statusCode),n.emit("error",y)),R();else if(y.statusCode&&(200>y.statusCode||400<=y.statusCode)){var Q=new q.MinigetError(`Status code: ${y.statusCode}`,y.statusCode);500<=y.statusCode?r(Q):n.emit("error",Q);R()}else{w=y;if(z.acceptEncoding&&
y.headers["content-encoding"])for(Q of y.headers["content-encoding"].split(", ").reverse()){let Y=z.acceptEncoding[Q];Y&&(w=w.pipe(Y()),w.on("error",r))}N||(N=parseInt(`${y.headers["content-length"]}`,10),M="bytes"===y.headers["accept-ranges"]&&0<N&&0<z.maxReconnects);y.on("data",P);w.on("end",F);w.pipe(n,{end:!M});p=y;n.emit("response",y);y.on("error",r);U(y,f)}});B.on("error",r);B.on("close",K);U(B,g);n.destroyed&&d(...a);n.emit("request",B);B.end()}else n.emit("error",new q.MinigetError(`Invalid URL: ${l}`))};
...
function(l){return l&&l.__esModule?l:{"default":l}};const A=h(x("http"));h=h(x("https"));const e=x("stream"),u={"http:":A.default,"https:":h.default},v=new Set([301,302,303,307,308]),m=new Set([429,503]),g="connect continue information socket timeout upgrade".split(" "),f=["aborted"];q.MinigetError=class extends Error{constructor(l,E){super(l);this.statusCode=E}};q.defaultOptions={maxRedirects:10,maxRetries:2,maxReconnects:0,backoff:{inc:100,max:1E4}};J.exports=q}).call(this)}).call(this,x("_process"))},
{_process:27,http:51,https:18,stream:36}],27:[function(x,J,D){function C(){throw Error("setTimeout has not been defined");}function q(){throw Error("clearTimeout has not been defined");}function h(n){if(g===setTimeout)return setTimeout(n,0);if((g===C||!g)&&setTimeout)return g=setTimeout,setTimeout(n,0);try{return g(n,0)}catch(B){try{return g.call(null,n,0)}catch(p){return g.call(this,n,0)}}}function A(n){if(f===clearTimeout)return clearTimeout(n);if((f===q||!f)&&clearTimeout)return f=clearTimeout,
clearTimeout(n);try{return f(n)}catch(B){try{return f.call(null,n)}catch(p){return f.call(this,n)}}}function e(){E&&t&&(E=!1,t.length?l=t.concat(l):z=-1,l.length&&u())}function u(){if(!E){var n=h(e);E=!0;for(var B=l.length;B;){t=l;for(l=[];++z<B;)t&&t[z].run();z=-1;B=l.length}t=null;E=!1;A(n)}}function v(n,B){this.fun=n;this.array=B}function m(){}x=J.exports={};try{var g="function"===typeof setTimeout?setTimeout:C}catch(n){g=C}try{var f="function"===typeof clearTimeout?clearTimeout:q}catch(n){f=q}var l=
[],E=!1,t,z=-1;x.nextTick=function(n){var B=Array(arguments.length-1);if(1<arguments.length)for(var p=1;p<arguments.length;p++)B[p-1]=arguments[p];l.push(new v(n,B));1!==l.length||E||h(u)};v.prototype.run=function(){this.fun.apply(null,this.array)};x.title="browser";x.browser=!0;x.env={};x.argv=[];x.version="";x.versions={};x.on=m;x.addListener=m;x.once=m;x.off=m;x.removeListener=m;x.removeAllListeners=m;x.emit=m;x.prependListener=m;x.prependOnceListener=m;x.listeners=function(n){return[]};x.binding=
function(n){throw Error("process.binding is not supported");};x.cwd=function(){return"/"};x.chdir=function(n){throw Error("process.chdir is not supported");};x.umask=function(){return 0}},{}],28:[function(x,J,D){(function(C){(function(){(function(q){function h(L){throw new RangeError(p[L]);}function A(L,I){for(var N=L.length,M=[];N--;)M[N]=I(L[N]);return M}function e(L,I){var N=L.split("@"),M="";1<N.length&&(M=N[0]+"@",L=N[1]);L=L.replace(B,".");L=L.split(".");I=A(L,I).join(".");return M+I}function u(L){for(var I=
[],N=0,M=L.length,T,S;N<M;)T=L.charCodeAt(N++),55296<=T&&56319>=T&&N<M?(S=L.charCodeAt(N++),56320==(S&64512)?I.push(((T&1023)<<10)+(S&1023)+65536):(I.push(T),N--)):I.push(T);return I}function v(L){return A(L,function(I){var N="";65535<I&&(I-=65536,N+=G(I>>>10&1023|55296),I=56320|I&1023);return N+=G(I)}).join("")}function m(L,I,N){var M=0;L=N?w(L/700):L>>1;for(L+=w(L/I);455<L;M+=36)L=w(L/35);return w(M+36*L/(L+38))}function g(L){var I=[],N=L.length,M=0,T=128,S=72,O,V;var W=L.lastIndexOf("-");0>W&&
(W=0);for(O=0;O<W;++O)128<=L.charCodeAt(O)&&h("not-basic"),I.push(L.charCodeAt(O));for(W=0<W?W+1:0;W<N;){O=M;var U=1;for(V=36;;V+=36){W>=N&&h("invalid-input");var b=L.charCodeAt(W++);b=10>b-48?b-22:26>b-65?b-65:26>b-97?b-97:36;(36<=b||b>w((2147483647-M)/U))&&h("overflow");M+=b*U;var a=V<=S?1:V>=S+26?26:V-S;if(b<a)break;b=36-a;U>w(2147483647/b)&&h("overflow");U*=b}U=I.length+1;S=m(M-O,U,0==O);w(M/U)>2147483647-T&&h("overflow");T+=w(M/U);M%=U;I.splice(M++,0,T)}return v(I)}function f(L){var I,N,M,T=
[];L=u(L);var S=L.length;var O=128;var V=0;var W=72;for(M=0;M<S;++M){var U=L[M];128>U&&T.push(G(U))}for((I=N=T.length)&&T.push("-");I<S;){var b=2147483647;for(M=0;M<S;++M)U=L[M],U>=O&&U<b&&(b=U);var a=I+1;b-O>w((2147483647-V)/a)&&h("overflow");V+=(b-O)*a;O=b;for(M=0;M<S;++M)if(U=L[M],U<O&&2147483647<++V&&h("overflow"),U==O){var d=V;for(b=36;;b+=36){U=b<=W?1:b>=W+26?26:b-W;if(d<U)break;var c=d-U;d=36-U;var k=T;U+=c%d;k.push.call(k,G(U+22+75*(26>U)-0));d=w(c/d)}T.push(G(d+22+75*(26>d)-0));W=m(V,a,I==
N);V=0;++I}++V;++O}return T.join("")}var l="object"==typeof D&&D&&!D.nodeType&&D,E="object"==typeof J&&J&&!J.nodeType&&J,t="object"==typeof C&&C;if(t.global===t||t.window===t||t.self===t)q=t;var z=/^xn--/,n=/[^\x20-\x7E]/,B=/[\x2E\u3002\uFF0E\uFF61]/g,p={overflow:"Overflow: input needs wider integers to process","not-basic":"Illegal input >= 0x80 (not a basic code point)","invalid-input":"Invalid input"},w=Math.floor,G=String.fromCharCode,H;t={version:"1.4.1",ucs2:{decode:u,encode:v},decode:g,encode:f,
toASCII:function(L){return e(L,function(I){return n.test(I)?"xn--"+f(I):I})},toUnicode:function(L){return e(L,function(I){return z.test(I)?g(I.slice(4).toLowerCase()):I})}};if(l&&E)if(J.exports==l)E.exports=t;else for(H in t)t.hasOwnProperty(H)&&(l[H]=t[H]);else q.punycode=t})(this)}).call(this)}).call(this,"undefined"!==typeof global?global:"undefined"!==typeof self?self:"undefined"!==typeof window?window:{})},{}],29:[function(x,J,D){J.exports=function(q,h,A,e){A=A||"=";var u={};if("string"!==typeof q||
0===q.length)return u;var v=/\+/g;q=q.split(h||"&");h=1E3;e&&"number"===typeof e.maxKeys&&(h=e.maxKeys);e=q.length;0<h&&e>h&&(e=h);for(h=0;h<e;++h){var m=q[h].replace(v,"%20"),g=m.indexOf(A);if(0<=g){var f=m.substr(0,g);m=m.substr(g+1)}else f=m,m="";f=decodeURIComponent(f);m=decodeURIComponent(m);Object.prototype.hasOwnProperty.call(u,f)?C(u[f])?u[f].push(m):u[f]=[u[f],m]:u[f]=m}return u};var C=Array.isArray||function(q){return"[object Array]"===Object.prototype.toString.call(q)}},{}],30:[function(x,
J,D){function C(e,u){if(e.map)return e.map(u);for(var v=[],m=0;m<e.length;m++)v.push(u(e[m],m));return v}var q=function(e){switch(typeof e){case "string":return e;case "boolean":return e?"true":"false";case "number":return isFinite(e)?e:"";default:return""}};J.exports=function(e,u,v,m){u=u||"&";v=v||"=";null===e&&(e=void 0);return"object"===typeof e?C(A(e),function(g){var f=encodeURIComponent(q(g))+v;return h(e[g])?C(e[g],function(l){return f+encodeURIComponent(q(l))}).join(u):f+encodeURIComponent(q(e[g]))}).join(u):
m?encodeURIComponent(q(m))+v+encodeURIComponent(q(e)):""};var h=Array.isArray||function(e){return"[object Array]"===Object.prototype.toString.call(e)},A=Object.keys||function(e){var u=[],v;for(v in e)Object.prototype.hasOwnProperty.call(e,v)&&u.push(v);return u}},{}],31:[function(x,J,D){D.decode=D.parse=x("./decode");D.encode=D.stringify=x("./encode")},{"./decode":29,"./encode":30}],32:[function(x,J,D){x=function(C){function q(b,a,d){Object.defineProperty(b,a,{value:d,enumerable:!0,configurable:!0,
writable:!0});return b[a]}function h(b,a,d,c){a=Object.create((a&&a.prototype instanceof e?a:e).prototype);c=new z(c||[]);a._invoke=f(b,d,c);return a}function A(b,a,d){try{return{type:"normal",arg:b.call(a,d)}}catch(c){return{type:"throw",arg:c}}}function e(){}function u(){}function v(){}function m(b){["next","throw","return"].forEach(function(a){q(b,a,function(d){return this._invoke(a,d)})})}function g(b,a){function d(k,r,K,R){k=A(b[k],b,r);if("throw"===k.type)R(k.arg);else{var P=k.arg;return(k=
P.value)&&"object"===typeof k&&w.call(k,"__await")?a.resolve(k.__await).then(function(F){d("next",F,K,R)},function(F){d("throw",F,K,R)}):a.resolve(k).then(function(F){P.value=F;K(P)},function(F){return d("throw",F,K,R)})}}var c;this._invoke=function(k,r){function K(){return new a(function(R,P){d(k,r,R,P)})}return c=c?c.then(K,K):K()}}function f(b,a,d){var c=M;return function(k,r){if(c===S)throw Error("Generator is already running");if(c===O){if("throw"===k)throw r;return B()}d.method=k;for(d.arg=
r;;){if(k=d.delegate)if(k=l(k,d)){if(k===V)continue;return k}if("next"===d.method)d.sent=d._sent=d.arg;else if("throw"===d.method){if(c===M)throw c=O,d.arg;d.dispatchException(d.arg)}else"return"===d.method&&d.abrupt("return",d.arg);c=S;k=A(b,a,d);if("normal"===k.type){c=d.done?O:T;if(k.arg===V)continue;return{value:k.arg,done:d.done}}"throw"===k.type&&(c=O,d.method="throw",d.arg=k.arg)}}}function l(b,a){var d=b.iterator[a.method];if(d===G){a.delegate=null;if("throw"===a.method){if(b.iterator["return"]&&
(a.method="return",a.arg=G,l(b,a),"throw"===a.method))return V;a.method="throw";a.arg=new TypeError("The iterator does not provide a 'throw' method")}return V}d=A(d,b.iterator,a.arg);if("throw"===d.type)return a.method="throw",a.arg=d.arg,a.delegate=null,V;d=d.arg;if(!d)return a.method="throw",a.arg=new TypeError("iterator result is not an object"),a.delegate=null,V;if(d.done)a[b.resultName]=d.value,a.next=b.nextLoc,"return"!==a.method&&(a.method="next",a.arg=G);else return d;a.delegate=null;return V}
function E(b){var a={tryLoc:b[0]};1 in b&&(a.catchLoc=b[1]);2 in b&&(a.finallyLoc=b[2],a.afterLoc=b[3]);this.tryEntries.push(a)}function t(b){var a=b.completion||{};a.type="normal";delete a.arg;b.completion=a}function z(b){this.tryEntries=[{tryLoc:"root"}];b.forEach(E,this);this.reset(!0)}function n(b){if(b){var a=b[L];if(a)return a.call(b);if("function"===typeof b.next)return b;if(!isNaN(b.length)){var d=-1;a=function k(){for(;++d<b.length;)if(w.call(b,d))return k.value=b[d],k.done=!1,k;k.value=
...
this._parser.onerror=function(K){r.emit("error",K);r._parser.error=null};this._decoder=null;I.forEach(function(K){Object.defineProperty(r,"on"+K,{get:function(){return r._parser["on"+K]},set:function(R){if(!R)return r.removeAllListeners(K),r._parser["on"+K]=R;r.on(K,R)},enumerable:!0,configurable:!1})})}function e(c){return" "===c||"\n"===c||"\r"===c||"\t"===c}function u(c){return'"'===c||"'"===c}function v(c,k,r){c.textNode&&m(c);c[k]&&c[k](r)}function m(c){c.textNode=g(c.opt,c.textNode);c.textNode&&
c.ontext&&c.ontext(c.textNode);c.textNode=""}function g(c,k){c.trim&&(k=k.trim());c.normalize&&(k=k.replace(/\s+/g," "));return k}function f(c,k){m(c);c.trackPosition&&(k+="\nLine: "+c.line+"\nColumn: "+c.column+"\nChar: "+c.c);k=Error(k);c.error=k;c.onerror&&c.onerror(k);return c}function l(c){c.sawRoot&&!c.closedRoot&&E(c,"Unclosed root tag");c.state!==a.BEGIN&&c.state!==a.BEGIN_WHITESPACE&&c.state!==a.TEXT&&f(c,"Unexpected end");m(c);c.c="";c.closed=!0;c.onend&&c.onend(void 0);h.call(c,c.strict,
c.opt);return c}function E(c,k){if("object"!==typeof c||!(c instanceof h))throw Error("bad call to strictFail");c.strict&&f(c,k)}function t(c,k){var r=0>c.indexOf(":")?["",c]:c.split(":"),K=r[0];r=r[1];k&&"xmlns"===c&&(K="xmlns",r="");return{prefix:K,local:r}}function z(c){c.strict||(c.attribName=c.attribName[c.looseCase]());if(-1===c.attribList.indexOf(c.attribName)&&!c.tag.attributes.hasOwnProperty(c.attribName))if(c.opt.xmlns){var k=t(c.attribName,!0),r=k.local;if("xmlns"===k.prefix)if("xml"===
...
r;r===this.q&&(this.state=a.DOCTYPE_DTD,this.q="");continue;case a.COMMENT:"-"===r?this.state=a.COMMENT_ENDING:this.comment+=r;continue;case a.COMMENT_ENDING:"-"===r?(this.state=a.COMMENT_ENDED,(this.comment=g(this.opt,this.comment))&&v(this,"oncomment",this.comment),this.comment=""):(this.comment+="-"+r,this.state=a.COMMENT);continue;case a.COMMENT_ENDED:">"!==r?(E(this,"Malformed comment"),this.comment+="--"+r,this.state=a.COMMENT):this.state=a.TEXT;continue;case a.CDATA:"]"===r?this.state=a.CDATA_ENDING:
...
10);r=k=0;for(P=H.length;r<P;r++){F=this[H[r]].length;if(F>c)switch(H[r]){case "textNode":m(this);break;case "cdata":v(this,"oncdata",this.cdata);this.cdata="";break;case "script":v(this,"onscript",this.script);this.script="";break;default:f(this,"Max buffer length exceeded: "+H[r])}k=Math.max(k,F)}this.bufferCheckPosition=q.MAX_BUFFER_LENGTH-k+this.position}return this},resume:function(){this.error=null;return this},close:function(){return this.write(null)},flush:function(){m(this);""!==this.cdata&&
