(() => {
  ...
  const handleError = (e) => {
    if (e instanceof TypeError) {
      console.log(
        "[error] YouTube proxy is currently having issues, please try again later."
      );
      alert(
        "YouTube proxy is currently having issues, please try again later."
      );
      return;
    }
    console.log(`[error] ${e}`);
    alert(
      "An error has occurred, please check the console output or DevTools."
    );
  };
  ...
  const getFormats = async () => {
    ...
    for (const [i, video] of videoList.entries()) {
      result += `<input type="radio" id="video-${i}" name="video" value="${i}"><label for="video-${i}">${
        video.qualityLabel
      } - ${video.fps} fps - ${video.mimeType}${
        !video.hasAudio ? " - video only" : ""
      }</label><br>`;
    }
  ...
  const getMedia = async () => {
    ...
    try {
      if (videoValue) {
        if (audioValue) {
          const buffer = await script.getVideoBuffer(
            videoList[parseInt(videoValue)],
            audioList[parseInt(audioValue)]
          );
          output.innerHTML =
            '<br><video id="player" width="100%" controls></video><br>';
        ...
        if (audioValue) {
          const buffer = await script.getAudioBuffer(
            audioList[parseInt(audioValue)]
          );
          output.innerHTML =
